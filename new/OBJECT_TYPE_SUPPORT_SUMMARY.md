...
- **Read:** ✅ Domain metadata (XML format)
- **Write:** ✅ Save and activate
- **Create:** ✅ `adt_create_domain`
- **Features:** Data type, length, decimals, value ranges
- **Format:** Returns XML with complete domain definition
- **Accept Header:** `application/vnd.sap.adt.domains.v2+xml`

**Endpoint:** `/sap/bc/adt/ddic/domains/{name}`

**Note:** Returns structured XML metadata, not DDL source code.

---

### 11. **Data Elements (DTEL/DATA_ELEMENT)** ✅
- **Read:** ✅ Data element metadata (XML format)
- **Write:** ✅ Save and activate
- **Create:** ✅ `adt_create_data_element`
- **Features:** Domain reference, field labels (short/medium/long/heading)
- **Format:** Returns XML with complete data element definition
...
- **Read:** ✅ Table type metadata (XML format)
- **Write:** ✅ Save and activate
- **Create:** ✅ `adt_create_table_type`
- **Types:** STANDARD, SORTED, HASHED
- **Features:** Row type definition, key configuration, access type
- **Format:** Returns XML with complete table type definition
...
**XML-Based Metadata:**
- Service Bindings (use `adt_read_service_binding`)
- Data Elements (DTEL) - Returns complete field definition with labels
- Domains (DOMA) - Returns type information and value constraints
- Table Types (TTYP) - Returns row type, access type, and key definitions
- Returns: Structured XML with metadata
...
### DDIC Objects Special Handling

**Data Elements, Domains, and Table Types** return XML metadata instead of source code:

**Data Element Example:**
```xml
<dtel:dataElement>
  <dtel:typeKind>domain</dtel:typeKind>
  <dtel:typeName>/COREVIST/BUSINESS_OBJECT</dtel:typeName>
  <dtel:dataType>CHAR</dtel:dataType>
  <dtel:dataTypeLength>000030</dtel:dataTypeLength>
  <dtel:shortFieldLabel>Bus. obj.</dtel:shortFieldLabel>
  <dtel:mediumFieldLabel>Business object</dtel:mediumFieldLabel>
  <dtel:longFieldLabel>Business object</dtel:longFieldLabel>
</dtel:dataElement>
```
...
<ttyp:rowType>
  ...
  <ttyp:typeName>/COREVIST/OBJECT_RECORD</ttyp:typeName>
  <ttyp:builtInType>
...
- **January 15, 2025:** Fixed Table Type (TTYP) reading to use proper XML endpoints
- **January 15, 2025:** Fixed Data Element (DTEL) and Domain (DOMA) reading to use proper XML endpoints
