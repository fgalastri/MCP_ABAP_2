...
### 6. `adt_where_used_list`

**Purpose:** Find where an ABAP object is being used (where-used list)

**Parameters:**
- `object_name` (string, required): Name of ABAP object (e.g., `ZCL_MY_CLASS`)
- `object_type` (string, required): Object type - `CLAS`, `INTF`, `PROG`, `DDLS`, `TABL`, `FUGR`, `DTEL`, `DOMA`
...
- List of all objects that reference the specified object
- Each reference includes: name, type, package, URI, usage information
...
**See:** `WHERE_USED_LIST_GUIDE.md` for complete documentation

---

### 7. `adt_unlock`

**Purpose:** Unlock ABAP objects without activating them

**Parameters:**
- `objects` (array, required): List of objects to unlock (supports batch operations)
  - Each object has:
    - `name` (string): Object name
    - `type` (string): Object type
    - `lock_handle` (string, optional): Lock handle from previous lock operation
...
**Returns:**
- Number of successfully unlocked objects
- List of objects that were already unlocked
- Error details if any unlock failed

**Use Cases:**
- Release locks on saved objects without activating
- Unlock objects locked in other sessions
- Batch unlock multiple objects
- Clean up locks after errors or interruptions

**Important:**
- ⚠️ Objects must be locked by the current user, or you need the lock handle
- ✅ Supports batch unlocking - unlock multiple objects in one call
- ✅ Gracefully handles already-unlocked objects (not an error)

---

### 8. `adt_reassign_package`

**Purpose:** Reassign an ABAP object to a new package (package refactoring)

**Parameters:**
- `object_name` (string, required): Name of ABAP object (e.g., `ZCL_MY_CLASS`, `/COREVIST/T_MAT_INC_EXC`)
- `object_type` (string, required): Object type - `CLAS`, `INTF`, `PROG`, `DDLS`, `BDEF`, `TABL`, `TTYP`, `DTEL`, `DOMA`, `STRUCT`, `SUSH`, `SCO3`
- `current_package` (string, required): Current package name (e.g., `/COREVIST/PRODUCT`)
- `new_package` (string, required): New package name (e.g., `/COREVIST/_PRODUCT`)
- `transport_request` (string, required): Transport request number (e.g., `H01K900086`)
- `description` (string, optional): Object description (auto-generated if not provided)
...
- Moving misplaced objects to correct packages
- Package consolidation and refactoring
- Automating package management

**Important:**
- ✅ Supports all major ABAP object types including CDS, BDEF, interfaces, classes, etc.
- ✅ Automatically handles namespace objects (e.g., `/COREVIST/...`)
- ✅ Validates authorizations and transport requests
- ⚠️ Local packages (starting with `$`) don't require transport requests
- ⚠️ Object must not be locked in another transport

**See:** `PACKAGE_REASSIGNMENT_GUIDE.md` for complete documentation with examples and error handling

---

### 9. `adt_remove_objects_from_transport`

**Purpose:** Remove objects from a transport request or task

**Parameters:**
- `transport_number` (string, required): Transport request or task number (e.g., `H01K900054`)
- `objects` (array, required): List of objects to remove from the transport
  - Each object has:
    - `name` (string, required): Object name (e.g., `/COREVIST/CLFN_PRODUCT_READER`)
    - `type` (string, required): Object type (e.g., `CLAS`, `TABL`, `GCPM`)
    - `pgmid` (string, optional): Program ID (default: `R3TR`)
    - `obj_desc` (string, optional): Object description
    - `position` (string, **IMPORTANT**): Position in transport (e.g., `000003`)
      - ⚠️ **The position attribute is critical for successful removal**
      - You must provide the object's actual position in the transport
      - To find the position, first GET the transport details to see where the object is located
...
**Returns:**
- Number of successfully removed objects
- Detailed status for each object (success/failure)
- Error details if any removal failed

**Use Cases:**
- Remove objects accidentally added to wrong transport
- Clean up transport requests before release
- Remove obsolete objects from transports
- Batch removal of multiple objects

**Important:**
- ⚠️ You must have authorization to modify the transport request/task
- ✅ Supports batch removal - remove multiple objects in one call
- ✅ Each object removal is independent - one failure doesn't stop others
- ⚠️ **BTP Only:** Currently only available in the BTP server (`abap-adt-btp`)

---

## 📖 Supported Object Types

| Type Code | Description | Read | Write | Activate | Package Reassign |
|-----------|-------------|------|-------|----------|-----------------|
...
| `BDEF` or `BEHAVIOR_DEFINITION` | Behavior Definition | ✅ | ✅ | ✅ | ✅ |
| `FUGR` or `FUNCTION_GROUP` | Function Group | ✅ | ✅ | ✅ | ❌ |
| `FUNC` or `FUNCTION_MODULE` | Function Module (with auto-discovery) | ✅ | ❌ | ⚠️ | ❌ |
| `TABL` or `TABLE` | Database Table | ✅ | ⚠️ | ✅ | ✅ |
| `TTYP` or `TABLE_TYPE` | Table Type | ✅ | ✅ | ✅ | ✅ |
| `DTEL` or `DATA_ELEMENT` | Data Element | ✅ | ✅ | ✅ | ✅ |
| `DOMA` or `DOMAIN` | Domain | ✅ | ✅ | ✅ | ✅ |
| `STRUCT` or `STRUCTURE` | Structure | ✅ | ✅ | ✅ | ✅ |
| `SUSH` or `AUTHORIZATION_DEFAULT` | Authorization Default | ❌ | ❌ | ❌ | ✅ |
| `SCO3` or `OUTBOUND_SERVICE` | Outbound Service | ❌ | ❌ | ❌ | ✅ |

**Notes:** 
- Function modules support **automatic function group discovery** - just provide the FM name!
- Structures use DDL format like tables
- Table structures might require special handling for write operations
- Package reassignment supports all major ABAP development object types
- Some object types like SUSH and SCO3 are only supported for package reassignment
...
### Example 4: Impact Analysis Before Changes
...
**Returns:**
- List of all objects that reference `ZCL_MY_CLASS`
- Each reference includes package, type, and usage information
- Use this before modifying or deleting objects to understand impact

---

### Example 5: Unlock Objects Without Activating
...
**Returns:**
- Successfully unlocked objects
- Objects that were already unlocked
- Use this when you've saved objects but want to unlock without activating

---

### Example 6: Reassign Package for an Object
...
**Returns:**
- Successfully reassigned object to new package
- Transport request information
- Use this to reorganize package structure or move misplaced objects

**See:** `PACKAGE_REASSIGNMENT_GUIDE.md` for comprehensive examples, error handling, and workflows

---

### Example 7: Remove Objects from Transport Request
...
**Returns:**
- Successfully removed objects from transport
- Detailed status for each object
- Use this to clean up transport requests or remove accidentally added objects

**Important Notes:**
- ⚠️ **The `position` attribute is critical for successful removal**
- You must specify the exact position of the object in the transport
- To find positions, first GET the transport to see the object list with positions
- Available only in BTP server (`abap-adt-btp`)
...
- **Where-Used List Guide:** `WHERE_USED_LIST_GUIDE.md` - Impact analysis tool
- **Package Reassignment Guide:** `PACKAGE_REASSIGNMENT_GUIDE.md` - Complete package refactoring guide
- **Transport Removal Guide:** `TRANSPORT_REMOVAL_GUIDE.md` - Remove objects from transports
