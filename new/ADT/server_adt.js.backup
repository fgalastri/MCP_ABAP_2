...
const SAP_CONFIG = {
  ...
  // BTP Authentication (optional - if provided, uses BTP auth instead of basic auth)
  authMode: process.env.SAP_AUTH_MODE || 'basic', // 'basic' or 'btp'
  oauthToken: process.env.SAP_OAUTH_TOKEN || null, // Bearer token for BTP systems
...
if (SAP_CONFIG.authMode === 'basic') {
  if (!SAP_CONFIG.username || !SAP_CONFIG.password) {
    console.error('❌ ERROR: SAP_USERNAME and SAP_PASSWORD are required for basic auth!');
    console.error('   Please check your MCP configuration in .cursor/mcp.json');
    process.exit(1);
  }
}
...
function loadBtpCookies() {
  try {
    if (SAP_CONFIG.authMode === 'btp' && fs.existsSync(SAP_CONFIG.btpCookiesFile)) {
      const cookiesData = JSON.parse(fs.readFileSync(SAP_CONFIG.btpCookiesFile, 'utf8'));
      ...
      if (cookies && cookies.length > 0) {
        log(`[CONFIG] Loaded ${cookies.length} cookies from ${SAP_CONFIG.btpCookiesFile}`);
        return cookies;
      }
    }
  } catch (error) {
...
class AdtService {
  constructor() {
    ...
    this.isBtpMode = SAP_CONFIG.authMode === 'btp';
    ...
    const axiosConfig = {
      ...
      headers: {
        ...
        'sap-language': SAP_CONFIG.language,
    ...
    if (this.isBtpMode) {
      ...
      if (SAP_CONFIG.oauthToken) {
        axiosConfig.headers['Authorization'] = `Bearer ${SAP_CONFIG.oauthToken}`;
      }
