CLASS zcl_xco_domain_creation DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_xco_domain_creation IMPLEMENTATION.
  METHOD create_domain.
    DATA: transport_request TYPE sxco_transport VALUE co_transport.

    TRY.
        IF xco_cp_abap_dictionary=>domain( co_domain_name )->exists( ) = abap_true.
          rv_message = |Domain { co_domain_name } already exists|.
          RETURN.
        ENDIF.

        DATA(lo_put_operation) = xco_cp_generation=>environment->dev_system( transport_request )->create_put_operation( ).

        DATA(lo_form_specification) = lo_put_operation->for-doma->add_object( co_domain_name
          )->set_package( co_package
          )->create_form_specification( ).

        lo_form_specification->set_short_description( 'XCO Demo Domain for Status Values' ).

        lo_form_specification->set_format( xco_cp_abap_dictionary=>built_in_type->char( 10 ) ).

        lo_form_specification->fixed_values->add_fixed_value( 'ACTIVE' )->set_description( 'Active Status' ).
        lo_form_specification->fixed_values->add_fixed_value( 'INACTIVE' )->set_description( 'Inactive Status' ).
        lo_form_specification->fixed_values->add_fixed_value( 'PENDING' )->set_description( 'Pending Status' ).
        lo_form_specification->fixed_values->add_fixed_value( 'BLOCKED' )->set_description( 'Blocked Status' ).
        ...
        IF lo_result->findings->contain_errors( ) = abap_true.
          ...
          ENDLOOP.
        ELSE.
          rv_message = |Domain { co_domain_name } created successfully|.
        ENDIF.
