...
# Find the H01K900021 task section
task_match = re.search(r'<tm:task tm:number="H01K900021".*?</tm:task>', content, re.DOTALL)
if task_match:
    ...
    print(f'Found {len(objects)} objects in H01K900021')
    print(f'\nTransport Owner: CB9980000020')
    print(f'Highest position: {objects[0]["position"]}')
    print(f'Lowest position: {objects[-1]["position"]}')
    
    # Write to a JSON file for easy processing
    import json
    with open('h01k900021_objects.json', 'w') as f:
        json.dump(objects, f, indent=2)
    
    print(f'\nObjects saved to h01k900021_objects.json')
    print(f'\nAll objects to remove (highest to lowest):')
    for obj in objects:
        desc = f' - {obj["obj_desc"][:40]}' if obj["obj_desc"] else ''
        print(f'  {obj["position"]}: {obj["name"]} ({obj["type"]}){desc}')
