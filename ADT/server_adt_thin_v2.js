...
// import axios from 'axios'; // REPLACED WITH THIN CLIENT
...
// Load active system from state file (for dynamic switching)
...
// Load SAP configuration from sap_systems.json
// Uses: 1) current_system.json, 2) SAP_SYSTEM env var, 3) defaults to 'DEV'
let ACTIVE_SYSTEM_KEY = loadActiveSystem();
let SAP_CONFIG = loadConfig(ACTIVE_SYSTEM_KEY);

// Create log file and logger based on system
let SERVER_ID = SAP_CONFIG.serverId;
let logFile = path.join(__dirname, `adt_debug_${SAP_CONFIG.systemKey.toLowerCase()}.log`);
...
class AdtService extends AdtServiceBase {
  ...
  async createTableType(tableTypeName, description, packageName, transportRequest, options = {}) {
    try {
      ...
      // Auto-detect ABAP language version based on system type
      const abapLanguageVersion = SAP_CONFIG.authMode === 'btp' ? 'cloudDevelopment' : 'standard';

      log(`[CREATE TABLE TYPE] Creating table type: ${tableTypeName}`);
      
      // BTP/Cloud uses PUT with lock handle (even for creation), On-premise uses POST
      const useBTPStyle = SAP_CONFIG.authMode === 'btp';
      
      if (useBTPStyle) {
        // ===== BTP/CLOUD STYLE: PUT with lock handle =====
        
        // Step 1: Lock the new table type (BTP requires lock even for creation)
        const lockResult = await this.lockObject(tableTypeName, 'TTYP');
        if (!lockResult.success) {
          throw new Error(`Failed to lock table type: ${lockResult.error}`);
        }
        
        const lockHandle = lockResult.lockHandle;
        log(`[CREATE TABLE TYPE] Lock acquired: ${lockHandle}`);
        ...
        try {
          ...
          // Determine if we have a line type to create complete definition
          let rowTypeXml = '';
          if (lineType) {
            rowTypeXml = `
  <ttyp:rowType>
  ...
  <ttyp:secondaryKeys ttyp:isVisible="true" ttyp:isEditable="true">
    <ttyp:allowed>notSpecified</ttyp:allowed>
          ...
          // Build complete table type XML (with version="new" for BTP creation)
          const xmlRequest = `<?xml version="1.0" encoding="UTF-8"?><ttyp:tableType xmlns:ttyp="http://www.sap.com/dictionary/tabletype" xmlns:adtcore="http://www.sap.com/adt/core" xmlns:atom="http://www.w3.org/2005/Atom" adtcore:description="${description}" adtcore:language="${language}" adtcore:name="${tableTypeName}" adtcore:type="TTYP/DA" adtcore:version="new" adtcore:abapLanguageVersion="${abapLanguageVersion}" adtcore:masterLanguage="${masterLanguage}" adtcore:responsible="${responsible}">
  <adtcore:packageRef adtcore:name="${packageName}"/>${rowTypeXml}
</ttyp:tableType>`;
          
          // Use PUT with lock handle (BTP/Cloud style creation)
          const encodedTableTypeName = encodeURIComponent(tableTypeName.toLowerCase());
          const createUrl = `/sap/bc/adt/ddic/tabletypes/${encodedTableTypeName}?lockHandle=${lockHandle}&corrNr=${transportRequest}`;
          ...
          log(`[CREATE TABLE TYPE] Table type created successfully: ${tableTypeName}`);
          
          // Unlock the table type
          await this.unlockObject(tableTypeName, 'TTYP', lockHandle);
          log(`[CREATE TABLE TYPE] Table type unlocked`);
          ...
          return {
            ...
            transportRequest,
            packageName,
            lineType: lineType ? lineType.toUpperCase() : null,
            tableCategory,
            message: lineType ? 'Table Type created with complete definition (BTP style)' : 'Table Type metadata created successfully (BTP style)',
          ...
          try {
            await this.unlockObject(tableTypeName, 'TTYP', lockHandle);
            log(`[CREATE TABLE TYPE] Table type unlocked after error`);
          } catch (unlockError) {
        ...
        // ===== ON-PREMISE STYLE: POST then PUT (if lineType provided) =====
        
        // Build XML request for table type creation (metadata only)
  ...
  <ttyp:secondaryKeys ttyp:isVisible="true" ttyp:isEditable="true">
    <ttyp:allowed>notSpecified</ttyp:allowed>
            ...
            const encodedTableTypeName = encodeURIComponent(tableTypeName.toLowerCase());
            const updateUrl = `/sap/bc/adt/ddic/tabletypes/${encodedTableTypeName}?lockHandle=${lockHandle}&corrNr=${transportRequest}`;
...
<dtel:wbobj xmlns:dtel="http://www.sap.com/wbobj/dictionary/dtel" xmlns:adtcore="http://www.sap.com/adt/core" adtcore:description="${description}" adtcore:language="${language}" adtcore:name="${dataElementName.toUpperCase()}" adtcore:type="DTEL/DE" adtcore:masterLanguage="${masterLanguage}" adtcore:masterSystem="S4H" adtcore:responsible="${responsible}">
  ...
  <dtel:dataElement>
    ...
    <dtel:deactivateBIDIFiltering>false</dtel:deactivateBIDIFiltering>
          ...
          // Use URL encoding for data element name (especially important for namespaced data elements like /COREVIST/xxx)
  ...
  /**
   * Save/Update a Domain with complete XML structure including fixed values
   * Handles lock → save → unlock workflow automatically
   * PUT /sap/bc/adt/ddic/domains/{name}?lockHandle={handle}&corrNr={transport}
   */
  async saveDomain(domainXml, transportRequest = null) {
    try {
      await this.getCsrfToken();
      
      // Parse the domain name from XML
      const domainNameMatch = domainXml.match(/adtcore:name="([^"]+)"/);
      if (!domainNameMatch) {
        throw new Error('Could not extract domain name from XML');
      }
      const domainName = domainNameMatch[1];
      
      log(`[SAVE DOMAIN] Saving domain: ${domainName}`);
      
      // Step 1: Lock the domain
      const lockResult = await this.lockObject(domainName, 'DOMA');
      if (!lockResult.success) {
        throw new Error(`Failed to lock domain: ${lockResult.error}`);
      }
      
      const lockHandle = lockResult.lockHandle;
      log(`[SAVE DOMAIN] Lock acquired: ${lockHandle}`);
      
      try {
        // Step 2: Save with lock handle
        // Use URL encoding for domain name (especially important for namespaced domains like /COREVIST/xxx)
        const encodedDomainName = encodeURIComponent(domainName.toLowerCase());
        let updateUrl = `/sap/bc/adt/ddic/domains/${encodedDomainName}?lockHandle=${lockHandle}`;
        if (transportRequest) {
        ...
        log(`[SAVE DOMAIN] Domain saved successfully: ${domainName}`);
        
        // Step 3: Unlock the domain
        await this.unlockObject(domainName, 'DOMA', lockHandle);
        log(`[SAVE DOMAIN] Domain unlocked`);

        return {
          success: true,
          domainName,
          message: 'Domain saved successfully (locked → saved → unlocked)',
          httpStatus: response.status,
        ...
        try {
          await this.unlockObject(domainName, 'DOMA', lockHandle);
          log(`[SAVE DOMAIN] Domain unlocked after error`);
        } catch (unlockError) {
          log(`[SAVE DOMAIN] Failed to unlock: ${unlockError.message}`);
        }
        throw error;
      }
    } catch (error) {
      log(`[SAVE DOMAIN] FAILED: ${error.message}`);
      log(`[SAVE DOMAIN] HTTP Status: ${error.response?.status}`);
  ...
  /**
   * Save/Update a Data Element with complete XML structure
   * Handles lock → save → unlock workflow automatically
   * PUT /sap/bc/adt/ddic/dataelements/{name}?lockHandle={handle}&corrNr={transport}
   */
  async saveDataElement(dataElementXml, transportRequest = null) {
    try {
      await this.getCsrfToken();
      
      // Parse the data element name from XML
      const dataElementNameMatch = dataElementXml.match(/adtcore:name="([^"]+)"/);
      if (!dataElementNameMatch) {
        throw new Error('Could not extract data element name from XML');
      }
      const dataElementName = dataElementNameMatch[1];
      
      log(`[SAVE DATA ELEMENT] Saving data element: ${dataElementName}`);
      
      // Step 1: Lock the data element
      const lockResult = await this.lockObject(dataElementName, 'DTEL');
      if (!lockResult.success) {
        throw new Error(`Failed to lock data element: ${lockResult.error}`);
      }
      
      const lockHandle = lockResult.lockHandle;
      log(`[SAVE DATA ELEMENT] Lock acquired: ${lockHandle}`);
      
      try {
        // Step 2: Save with lock handle
        // Use URL encoding for data element name (especially important for namespaced data elements like /COREVIST/xxx)
        const encodedDataElementName = encodeURIComponent(dataElementName.toLowerCase());
        let updateUrl = `/sap/bc/adt/ddic/dataelements/${encodedDataElementName}?lockHandle=${lockHandle}`;
        if (transportRequest) {
          updateUrl += `&corrNr=${transportRequest}`;
        }
        ...
        log(`[SAVE DATA ELEMENT] Data element saved successfully: ${dataElementName}`);
        
        // Step 3: Unlock the data element
        await this.unlockObject(dataElementName, 'DTEL', lockHandle);
        log(`[SAVE DATA ELEMENT] Data element unlocked`);

        return {
          success: true,
          dataElementName,
          message: 'Data element saved successfully (locked → saved → unlocked)',
          httpStatus: response.status,
          responseXml: response.data
        };
      } catch (error) {
        ...
        try {
          await this.unlockObject(dataElementName, 'DTEL', lockHandle);
          log(`[SAVE DATA ELEMENT] Data element unlocked after error`);
        } catch (unlockError) {
          log(`[SAVE DATA ELEMENT] Failed to unlock: ${unlockError.message}`);
        }
        throw error;
      }
    } catch (error) {
      log(`[SAVE DATA ELEMENT] FAILED: ${error.message}`);
      log(`[SAVE DATA ELEMENT] HTTP Status: ${error.response?.status}`);
  ...
  /**
   * Save/Update a Table Type with complete XML structure
   * Automatically handles lock → save → unlock workflow
  ...
  async saveTableType(tableTypeXml, transportRequest = null) {
    try {
      ...
      // Parse the table type name from XML
      const tableTypeNameMatch = tableTypeXml.match(/adtcore:name="([^"]+)"/);
      if (!tableTypeNameMatch) {
        throw new Error('Could not extract table type name from XML');
      }
      const tableTypeName = tableTypeNameMatch[1];
      
      log(`[SAVE TABLE TYPE] Saving table type: ${tableTypeName}`);
      
      // Step 1: Lock the table type
      const lockResult = await this.lockObject(tableTypeName, 'TTYP');
      if (!lockResult.success) {
        throw new Error(`Failed to lock table type: ${lockResult.error}`);
      }
      ...
      const lockHandle = lockResult.lockHandle;
      log(`[SAVE TABLE TYPE] Lock acquired: ${lockHandle}`);
      
      try {
        // Step 2: Save with lock handle
        // Use URL encoding for table type name (especially important for namespaced table types like /COREVIST/xxx)
        const encodedTableTypeName = encodeURIComponent(tableTypeName.toLowerCase());
        let updateUrl = `/sap/bc/adt/ddic/tabletypes/${encodedTableTypeName}?lockHandle=${lockHandle}`;
        if (transportRequest) {
          updateUrl += `&corrNr=${transportRequest}`;
        }
        ...
        log(`[SAVE TABLE TYPE] Table type saved successfully: ${tableTypeName}`);
        
        // Step 3: Unlock the table type
        await this.unlockObject(tableTypeName, 'TTYP', lockHandle);
        log(`[SAVE TABLE TYPE] Table type unlocked`);

        return {
          success: true,
          tableTypeName,
          message: 'Table type saved successfully (locked → saved → unlocked)',
          httpStatus: response.status,
          responseXml: response.data
        };
      } catch (error) {
        ...
        try {
          await this.unlockObject(tableTypeName, 'TTYP', lockHandle);
          log(`[SAVE TABLE TYPE] Table type unlocked after error`);
        } catch (unlockError) {
          log(`[SAVE TABLE TYPE] Failed to unlock: ${unlockError.message}`);
        }
        throw error;
      }
    } catch (error) {
      log(`[SAVE TABLE TYPE] FAILED: ${error.message}`);
      log(`[SAVE TABLE TYPE] HTTP Status: ${error.response?.status}`);
  ...
  /**
   * List objects in a package using virtual folders
   * POST /sap/bc/adt/repository/informationsystem/virtualfolders/contents
   * Can filter by object type (DTEL, CLAS, DOMA, etc.) or get all objects
   */
  async listPackageObjects(packageName, objectType = null) {
    try {
      await this.getCsrfToken();
      
      log(`[LIST PACKAGE OBJECTS] Package: ${packageName}, Type: ${objectType || 'ALL'}`);
...
<vfs:virtualFoldersRequest xmlns:vfs="http://www.sap.com/adt/ris/virtualFolders" objectSearchPattern="*">
  <vfs:preselection facet="package">
    <vfs:value>${packageName}</vfs:value>
      ...
      if (objectType) {
        // Map common object type names to ADT types
        const typeMapping = {
          'CLAS': 'CLAS',
          'CLASS': 'CLAS',
          'INTF': 'INTF',
        ...
        const adtType = typeMapping[objectType.toUpperCase()] || objectType.toUpperCase();
        
        // Determine group based on object type
        let group = null;
        if (['DTEL', 'DOMA', 'TABL', 'TTYP'].includes(adtType)) {
          group = 'DICTIONARY';
        } else if (['DDLS', 'BDEF'].includes(adtType)) {
          group = 'CORE_DATA_SERVICES';
        } else if (['CLAS', 'INTF'].includes(adtType)) {
          group = 'SOURCE_LIBRARY';
        } else if (['PROG', 'FUGR'].includes(adtType)) {
          group = 'PROGRAMS';
        }
  ...
  <vfs:preselection facet="group">
    <vfs:value>${group}</vfs:value>
  </vfs:preselection>`;
        }
        
        xmlRequest += `
  <vfs:preselection facet="type">
    <vfs:value>${adtType}</vfs:value>
  </vfs:preselection>`;
      }
  ...
  <vfs:facetorder/>
      ...
      log(`[LIST PACKAGE OBJECTS] Response status: ${response.status}`);
      ...
      const virtualFoldersResult = result['vfs:virtualFoldersResult'];
      
      const objectCount = parseInt(virtualFoldersResult['@_objectCount']) || 0;
      const objects = [];
      
      if (objectCount > 0) {
        const vfsObjects = virtualFoldersResult['vfs:object'];
        const objectArray = Array.isArray(vfsObjects) ? vfsObjects : [vfsObjects];
        
        for (const obj of objectArray) {
          objects.push({
            name: obj['@_name'],
            type: obj['@_type'],
            description: obj['@_text'],
      ...
      log(`[LIST PACKAGE OBJECTS] Found ${objects.length} objects`);
      ...
      log(`[LIST PACKAGE OBJECTS] FAILED: ${error.message}`);
      log(`[LIST PACKAGE OBJECTS] HTTP Status: ${error.response?.status}`);
  ...
  async readSource(objectName, objectType, functionGroup = null) {
    try {
      ...
      // For interfaces, add version=workingArea parameter to the source URI
      let finalUri = uri;
      if (objTypeUpper === 'INTF' || objTypeUpper === 'INTERFACE') {
        finalUri = `${uri}?version=workingArea`;
        // Keep text/plain for ABAP source code
      }
      ...
      // INTF uses text/plain for source code (like classes)
      
      const response = await this.client.get(finalUri, {
        headers: { 'Accept': acceptHeader }
      });

      // For DDIC objects (DTEL, DOMA, TTYP), parse XML and format it nicely
      if (objTypeUpper === 'DTEL' || objTypeUpper === 'DATA_ELEMENT' || 
          objTypeUpper === 'DOMA' || objTypeUpper === 'DOMAIN' ||
          objTypeUpper === 'TTYP' || objTypeUpper === 'TABLE_TYPE') {
        // Return the raw XML for now - we can enhance formatting later
        return {
          success: true,
...
<class:abapClass xmlns:class="http://www.sap.com/adt/oo/classes" xmlns:adtcore="http://www.sap.com/adt/core" adtcore:description="${description}" adtcore:language="${language}" adtcore:name="${className.toUpperCase()}" adtcore:type="CLAS/OC" adtcore:masterLanguage="${masterLanguage}" adtcore:masterSystem="S4H" adtcore:responsible="${responsible}" class:final="${final}" class:visibility="${visibility}">
   ...
   */
  async runUnitTests(objectName, objectType, client = null) {
    try {
      await this.getCsrfToken();
      ...
      log(`[RUN TESTS] 🎯 Client: ${client || SAP_CONFIG.client} ${client ? '(override)' : '(default)'}`);
  ...
  <adtcore:objectSets xmlns:adtcore="http://www.sap.com/adt/core">
    <objectSet kind="inclusive">
      ...
      // Build headers and request config
      const headers = {
      ...
      // If client override is specified, use client-specific credentials if available
      if (client) {
        headers['sap-client'] = client;
        
        // Check for client-specific credentials from config
        const clientCreds = SAP_CONFIG.clientSpecificCredentials[client];
        const clientUsername = clientCreds?.username;
        const clientPassword = clientCreds?.password;
        
        if (clientUsername && clientPassword) {
          // Get fresh CSRF token for this client
          log(`[RUN TESTS] Getting fresh CSRF token for client ${client} with credentials ${clientUsername}`);
          const { csrfToken, cookies } = await this.getClientSpecificCsrfToken(client, clientUsername, clientPassword);
          
          // Use client-specific credentials and CSRF token
          log(`[RUN TESTS] Using client-specific session`);
          
          // CRITICAL: Build Authorization header
          const authString = Buffer.from(`${clientUsername}:${clientPassword}`).toString('base64');
          headers['Authorization'] = `Basic ${authString}`;
          
          headers['X-CSRF-Token'] = csrfToken;
          
          // Use cookies from the CSRF token request
          if (cookies && cookies.length > 0) {
            const cookieStrings = cookies.map(c => c.split(';')[0]);
            headers['Cookie'] = cookieStrings.join('; ');
            log(`[RUN TESTS] Using session cookies: ${headers['Cookie']}`);
          }
          
          log(`[RUN TESTS] 🔐 Auth header set for client ${client}`);
          log(`[RUN TESTS] Headers: ${JSON.stringify(headers, null, 2)}`);
          
          // CRITICAL: Use fresh axios instance to avoid session contamination
          ...
          const response = await freshAxios.post(runUrl, testConfig, { headers });
          ...
          // Use default credentials - get CSRF token normally
          await this.getCsrfToken();
          log(`[RUN TESTS] Using default credentials for client ${client}`);
          headers['X-CSRF-Token'] = this.csrfToken;
          
          if (this.cookies && this.cookies.has('sap-usercontext')) {
            const userContext = this.cookies.get('sap-usercontext');
            const updatedContext = userContext.replace(/sap-client=\d+/, `sap-client=${client}`);
            headers['Cookie'] = updatedContext;
          }
        }
      } else {
        // Default client - use CSRF token normally
        await this.getCsrfToken();
        headers['X-CSRF-Token'] = this.csrfToken;
      }

      const response = await this.client.post(runUrl, testConfig, requestConfig);
  ...
  /**
   * Get CSRF token for a specific client with specific credentials
   * Creates a fresh session with the provided credentials
   * IMPORTANT: Uses a fresh axios instance to avoid session contamination
   */
  async getClientSpecificCsrfToken(client, username, password) {
    try {
      log(`[GET CLIENT CSRF] Fetching CSRF token for client ${client} with username ${username}`);
      
      // CRITICAL: Create a fresh axios instance WITHOUT shared session state
      // This prevents client 210 cookies from interfering with client 220 request
      const freshAxios = axios.create({
        baseURL: SAP_CONFIG.baseUrl,
        timeout: 300000,
        headers: {
      ...
      log(`[GET CLIENT CSRF] Making fresh request to /sap/bc/adt/discovery`);
      log(`[GET CLIENT CSRF] Headers: ${JSON.stringify(headers, null, 2)}`);
      
      // Make GET request with client-specific credentials (NO shared session)
      ...
      log(`[GET CLIENT CSRF] SUCCESS: Got CSRF token for client ${client}`);
      log(`[GET CLIENT CSRF] Token: ${csrfToken}`);
      log(`[GET CLIENT CSRF] Cookies count: ${cookies.length}`);
      log(`[GET CLIENT CSRF] Cookies: ${JSON.stringify(cookies, null, 2)}`);
      ...
      log(`[GET CLIENT CSRF] FAILED: ${error.message}`);
      throw error;
    }
  }
  ...
  async executeClass(className, client = null) {
    try {
      ...
      log(`[EXECUTE CLASS] 🎯 Client: ${client || SAP_CONFIG.client} ${client ? '(override)' : '(default)'}`);
      ...
      if (client) {
        ...
        if (clientUsername && clientPassword) {
          // Get fresh CSRF token for this client
          log(`[EXECUTE CLASS] Getting fresh CSRF token for client ${client} with credentials ${clientUsername}`);
          const { csrfToken, cookies } = await this.getClientSpecificCsrfToken(client, clientUsername, clientPassword);
          
          // Use client-specific credentials and CSRF token
          log(`[EXECUTE CLASS] Using client-specific session`);
          ...
          // Use cookies from the CSRF token request to maintain session
          if (cookies && cookies.length > 0) {
            // Extract and combine cookies
            const cookieStrings = cookies.map(c => c.split(';')[0]);
            headers['Cookie'] = cookieStrings.join('; ');
            log(`[EXECUTE CLASS] Using session cookies: ${headers['Cookie']}`);
          }
          
          log(`[EXECUTE CLASS] 🔐 Auth header set for client ${client}`);
          log(`[EXECUTE CLASS] Headers: ${JSON.stringify(headers, null, 2)}`);
          ...
          const response = await freshAxios.post(executeUrl, '', { headers });
          ...
          // Use default credentials - get CSRF token normally
          await this.getCsrfToken();
          log(`[EXECUTE CLASS] Using default credentials for client ${client}`);
          headers['X-CSRF-Token'] = this.csrfToken;
      ...
      const response = await this.client.post(executeUrl, '', requestConfig);
  ...
  /**
   * Execute SQL Query - Direct Data Preview (F8 functionality)
   * Endpoint: POST /sap/bc/adt/datapreview/freestyle?rowNumber={maxRows}
   * Executes raw SELECT statements and returns results with metadata
   */
  async executeSqlQuery(sqlQuery, maxRows = 100, client = null) {
    try {
      log(`[SQL QUERY] Executing SQL query`);
      log(`[SQL QUERY] Max rows: ${maxRows}`);
      log(`[SQL QUERY] 🎯 Client: ${client || SAP_CONFIG.client} ${client ? '(override)' : '(default)'}`);
      log(`[SQL QUERY] Query length: ${sqlQuery.length} characters`);

      // Security: Verify this is a SELECT statement (READ-ONLY)
      const normalizedQuery = sqlQuery.trim().toUpperCase();
      if (!normalizedQuery.startsWith('SELECT')) {
        log(`[SQL QUERY] SECURITY VIOLATION: Non-SELECT statement rejected`);
        return {
          success: false,
          error: 'Security Error: Only SELECT statements are allowed',
          hint: 'This tool is read-only. Use SELECT statements only.',
          sqlQuery: sqlQuery.substring(0, 100) + '...'
        };
      }

      // Build the Data Preview API endpoint
      ...
      if (client) {
        ...
        if (clientUsername && clientPassword) {
          // Get fresh CSRF token for this client
          log(`[SQL QUERY] Getting fresh CSRF token for client ${client} with credentials ${clientUsername}`);
          const { csrfToken, cookies } = await this.getClientSpecificCsrfToken(client, clientUsername, clientPassword);
          
          // Use client-specific credentials and CSRF token
          log(`[SQL QUERY] Using client-specific session`);
          ...
          if (cookies && cookies.length > 0) {
            ...
            const cookieStrings = cookies.map(c => c.split(';')[0]);
            headers['Cookie'] = cookieStrings.join('; ');
            log(`[SQL QUERY] Using session cookies: ${headers['Cookie']}`);
          }
          
          log(`[SQL QUERY] 🔐 Auth header set for client ${client}`);
          ...
          // Execute the query with SQL in body
          const response = await freshAxios.post(queryUrl, sqlQuery, { headers });
          
          log(`[SQL QUERY] Response status ${response.status}`);
          log(`[SQL QUERY] Response length: ${response.data?.length || 0} characters`);
          ...
          // Use default credentials - get CSRF token normally
          await this.getCsrfToken();
          log(`[SQL QUERY] Using default credentials for client ${client}`);
          headers['X-CSRF-Token'] = this.csrfToken;
      ...
      // Execute the query with SQL in body
      const response = await this.client.post(queryUrl, sqlQuery, requestConfig);
      ...
      return {
        ...
        httpStatus: error.response?.status,
        details: errorDetails.details,
        hint: errorDetails.hint || 'Check SQL syntax and ensure you have authorization to read the data',
        rawError: error.response?.data,
  ...
  /**
   * Parse SQL Query Response (Data Preview XML)
   * Converts the ADT Data Preview XML format to structured JSON
   */
  parseSqlQueryResponse(xmlData, originalQuery) {
    try {
      // Parse the XML with proper namespace handling
      const parser = new XMLParser({
        ignoreAttributes: false,
        attributeNamePrefix: '@_',
        removeNSPrefix: true,  // Remove namespace prefixes (dataPreview:)
        textNodeName: '#text',
        parseTagValue: false,
      ...
      // Navigate to the table data (namespace prefix removed by removeNSPrefix)
      ...
      // Extract metadata (namespace prefix removed by removeNSPrefix)
      ...
      // Extract columns (namespace prefix removed by removeNSPrefix)
      ...
      for (const col of columnsArray) {
        const metadata = col['metadata'];  // namespace prefix removed
        const dataSet = col['dataSet'];    // namespace prefix removed
        
        if (metadata) {
          // Attributes are now prefixed with @_
          columns.push({
            name: metadata['@_name'] || '',
            type: metadata['@_type'] || '',
  ...
  /**
   * Add objects to a transport request
   * Endpoint: PUT /sap/bc/adt/cts/transportrequests/{transport}
   * Used in BTP where objects are not automatically added during save
   */
  async addObjectsToTransport(transportNumber, objects) {
    try {
      await this.getCsrfToken();
      
      log(`[ADD TO TRANSPORT] Adding ${objects.length} object(s) to transport: ${transportNumber}`);
      ...
      // Iterate through objects and add them one by one
      const results = [];
      for (const obj of objects) {
        try {
          log(`[ADD TO TRANSPORT] Adding object: ${obj.name} (${obj.type}/${obj.pgmid || 'R3TR'})`);
          ...
          log(`[ADD TO TRANSPORT] XML Body: ${xmlBody}`);
          
          // Make PUT request to add the object
          const url = `/sap/bc/adt/cts/transportrequests/${transportNumber}`;
          ...
          log(`[ADD TO TRANSPORT] Object ${obj.name} added successfully. Status: ${response.status}`);
          log(`[ADD TO TRANSPORT] Response: ${JSON.stringify(response.data)}`);
          
          // Parse response to get position and lock status
          const parser = new XMLParser({ ignoreAttributes: false });
          const responseData = parser.parse(response.data);
          const abapObjectResponse = responseData?.['tm:root']?.['tm:request']?.['tm:abap_object'];
          
          results.push({
            object: obj.name,
            success: true,
            status: response.status,
            position: abapObjectResponse?.['@_tm:position'],
            lockStatus: abapObjectResponse?.['@_tm:lock_status']
          });
          
        } catch (error) {
          log(`[ADD TO TRANSPORT] FAILED for object ${obj.name}: ${error.message}`);
          log(`[ADD TO TRANSPORT] HTTP Status: ${error.response?.status}`);
      ...
      return {
        success: allSucceeded,
        message: allSucceeded 
          ? `✅ All ${successCount} object(s) added successfully to transport ${transportNumber}`
          : `⚠️  ${successCount} out of ${results.length} object(s) added successfully`,
        transportNumber,
        results
      };
      
    } catch (error) {
      log(`[ADD TO TRANSPORT] FAILED: ${error.message}`);
      log(`[ADD TO TRANSPORT] HTTP Status: ${error.response?.status}`);
      log(`[ADD TO TRANSPORT] Response Data: ${JSON.stringify(error.response?.data)}`);
...
// Create MCP server with name from config
const serverName = SAP_CONFIG.serverName;
...
log(`[MCP SERVER] ========================================`);
log(`[MCP SERVER] Registered MCP Server: ${serverName}`);
log(`[MCP SERVER] System: ${SAP_CONFIG.systemKey} (${SAP_CONFIG.serverId})`);
log(`[MCP SERVER] Base URL: ${SAP_CONFIG.baseUrl}`);
log(`[MCP SERVER] Client: ${SAP_CONFIG.client}`);
log(`[MCP SERVER] Auth Mode: ${SAP_CONFIG.authMode}`);
log(`[MCP SERVER] ========================================`);
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      {
        name: 'thin_v2_switch_system',
        description: prependAlwaysReadReminder('🔄 Switch between SAP systems (DEV/QA/BTP) without restarting Cursor. All subsequent tool calls will use the selected system.'),
        inputSchema: {
          type: 'object',
          properties: {
            system: {
              ...
              description: 'Target system to switch to: DEV (Development), QA (Quality Assurance), or BTP (Cloud)'
            }
          },
          required: ['system']
        }
      ...
      {
        name: 'thin_v2_create_class',
        description: prependAlwaysReadReminder('Create a new ABAP class in SAP system (metadata only). This creates the class structure with package, description, and properties. Source code must be added separately with adt_save_source.'),
        inputSchema: {
          type: 'object',
          properties: {
            class_name: {
      ...
      {
        name: 'thin_v2_create_table',
        description: prependAlwaysReadReminder('Create a new database table (DDIC table) in SAP system. This creates the basic table structure. Fields must be added separately.'),
        inputSchema: {
          type: 'object',
          properties: {
            table_name: {
      ...
      {
        name: 'thin_v2_create_package',
        description: prependAlwaysReadReminder('Create a new ABAP Package (Development Package) in SAP system. Packages are containers for ABAP objects.'),
        inputSchema: {
          type: 'object',
          properties: {
            package_name: {
      ...
      {
        name: 'thin_v2_create_interface',
        description: prependAlwaysReadReminder('Create a new ABAP interface in SAP system. Interfaces define contracts for classes.'),
        inputSchema: {
          type: 'object',
          properties: {
            interface_name: {
      ...
      {
        name: 'thin_v2_create_program',
        description: prependAlwaysReadReminder('Create a new ABAP program/report in SAP system. Supports executable programs, includes, module pools, etc.'),
        inputSchema: {
          type: 'object',
          properties: {
            program_name: {
      ...
      {
        name: 'thin_v2_create_cds_view',
        description: prependAlwaysReadReminder('Create a new CDS View (Core Data Services) in SAP system with optional DDL source code. Used for modern data modeling in S/4HANA.'),
        inputSchema: {
          type: 'object',
          properties: {
            cds_name: {
      ...
      {
        name: 'thin_v2_create_data_element',
        description: prependAlwaysReadReminder('Create a new Data Element in SAP system with optional complete definition. If domain_name is provided, creates a fully defined data element with field labels.'),
        inputSchema: {
          type: 'object',
          properties: {
            data_element_name: {
      ...
      {
        name: 'thin_v2_create_domain',
        description: prependAlwaysReadReminder('Create a new Domain in SAP system with optional complete definition. If data_type is provided, creates a fully defined domain ready to activate.'),
        inputSchema: {
          type: 'object',
          properties: {
            domain_name: {
      ...
      {
        name: 'thin_v2_save_domain',
        description: prependAlwaysReadReminder('Save/Update a Domain with complete XML structure including data type, length, decimals, and fixed values. Automatically handles lock → save → unlock workflow.'),
        inputSchema: {
          type: 'object',
          properties: {
            domain_xml: {
              type: 'string',
              description: 'Complete domain XML structure including <doma:content> with typeInformation, outputInformation, and valueInformation (for fixed values)'
            },
            transport_request: {
              type: 'string',
              description: 'Optional: Transport request number (e.g., H01K900111). If not provided, uses the domain\'s current transport.'
            }
          },
          required: ['domain_xml']
        }
      ...
      {
        name: 'thin_v2_save_data_element',
        description: prependAlwaysReadReminder('Save/Update a Data Element with complete XML structure including domain reference and field labels. Automatically handles lock → save → unlock workflow.'),
        inputSchema: {
          type: 'object',
          properties: {
            data_element_xml: {
              type: 'string',
              description: 'Complete data element XML structure including <dtel:dataElement> with typeKind, typeName (domain), and field labels'
            },
            transport_request: {
              type: 'string',
              description: 'Optional: Transport request number (e.g., H01K900111). If not provided, uses the data element\'s current transport.'
            }
          },
          required: ['data_element_xml']
        }
      },
      {
        name: 'thin_v2_save_table_type',
        description: prependAlwaysReadReminder('Save/Update a Table Type with complete XML structure including row type, access type, and key definitions. Automatically handles lock → save → unlock workflow.'),
        inputSchema: {
          type: 'object',
          properties: {
            table_type_xml: {
              type: 'string',
              description: 'Complete table type XML structure including <ttyp:tableType> with rowType, accessType, primaryKey, and secondaryKeys'
            },
            transport_request: {
              type: 'string',
              description: 'Optional: Transport request number (e.g., H01K900111). If not provided, uses the table type\'s current transport.'
            }
          },
      ...
      {
        name: 'thin_v2_create_table_type',
        description: prependAlwaysReadReminder('Create a new Table Type in SAP system with optional complete definition. If line_type is provided, creates a fully defined table type ready to activate.'),
        inputSchema: {
          type: 'object',
          properties: {
            table_type_name: {
      ...
      {
        name: 'thin_v2_create_structure',
        description: prependAlwaysReadReminder('Create a new Structure in SAP system with optional field definitions. Structures are reusable data structures.'),
        inputSchema: {
          type: 'object',
          properties: {
            structure_name: {
      ...
      {
        name: 'thin_v2_create_service_definition',
        description: prependAlwaysReadReminder('Create a new Service Definition (SRVD) in SAP system. Service Definitions expose CDS entities as services.'),
        inputSchema: {
          type: 'object',
          properties: {
            service_name: {
      ...
      {
        name: 'thin_v2_create_service_binding',
        description: prependAlwaysReadReminder('Create a new Service Binding (SRVB) in SAP system. Service Bindings expose service definitions as OData endpoints.'),
        inputSchema: {
          type: 'object',
          properties: {
            binding_name: {
      ...
      {
        name: 'thin_v2_read_service_binding',
        description: prependAlwaysReadReminder('Read Service Binding metadata from SAP system. Returns detailed information about the service binding including service definition, binding type, published status, and more.'),
        inputSchema: {
          type: 'object',
          properties: {
            binding_name: {
      ...
      {
        name: 'thin_v2_create_behavior_definition',
        description: prependAlwaysReadReminder('Create a new Behavior Definition (BDEF) in SAP system. Behavior Definitions define the behavior of RAP business objects (CRUD operations, validations, actions).'),
        inputSchema: {
          type: 'object',
          properties: {
            bdef_name: {
      ...
      {
        name: 'thin_v2_create_metadata_extension',
        description: prependAlwaysReadReminder('Create a new Metadata Extension (DDLX) in SAP system. Metadata Extensions add UI annotations to CDS views for Fiori Elements applications.'),
        inputSchema: {
          type: 'object',
          properties: {
      ...
      {
        name: 'thin_v2_generate_rap_ui_service',
        description: prependAlwaysReadReminder('🚀 Generate complete RAP UI Service (Business Object) using Eclipse ADT Generator. Creates R-layer CDS, C-layer CDS, Behavior Definition/Implementation, Draft Table, Service Definition, and Service Binding (OData V4) - all in ONE call!'),
        inputSchema: {
          type: 'object',
          properties: {
            table_name: {
      ...
      {
        name: 'thin_v2_generate_custom_query',
        description: prependAlwaysReadReminder('🔍 Generate Custom Query (Abstract Entity + Query Provider). Creates a complete custom query implementation with Abstract Entity, Query Provider Class implementing if_rap_query_provider, and optional Service Definition. Perfect for external APIs, complex aggregations, or custom business logic!'),
        inputSchema: {
          type: 'object',
          properties: {
            entity_name: {
      ...
      },
      {
        ...
        description: prependAlwaysReadReminder('Read ABAP object source code from SAP system using ADT REST API. Works with classes, interfaces, programs, includes, CDS views, etc.'),
        inputSchema: {
          type: 'object',
          properties: {
            object_name: {
      ...
      {
        name: 'thin_v2_save_source',
        description: prependAlwaysReadReminder('Save ABAP object source code to SAP system. Workflow: Lock → Save → Syntax Check. Object remains LOCKED after save. Use adt_activate to unlock and activate. Supports batch workflow: save multiple objects, then activate all together.'),
        inputSchema: {
          type: 'object',
          properties: {
            object_name: {
      ...
      {
        name: 'thin_v2_save_testclass_source',
        description: prependAlwaysReadReminder('Save test class source code for a class. Workflow: Lock class → Save test class include → keep LOCKED. Use for saving /includes/testclasses content. Use adt_activate to unlock and activate.'),
        inputSchema: {
          type: 'object',
          properties: {
            class_name: {
      ...
      {
        name: 'thin_v2_read_testclass_source',
        description: prependAlwaysReadReminder('Read test class source code from a class. Returns the ABAP Unit test classes (ltcl_*) from /includes/testclasses. Test classes contain setup, test methods, and assertions.'),
        inputSchema: {
          type: 'object',
          properties: {
            class_name: {
      ...
      {
        name: 'thin_v2_read_local_implementations',
        description: prependAlwaysReadReminder('Read local implementations (handler/saver classes) from a class. Returns the source code of lhc_* and lsc_* classes from /includes/implementations. Used for RAP behavior handler classes.'),
        inputSchema: {
          type: 'object',
          properties: {
            class_name: {
      ...
      {
        name: 'thin_v2_save_local_implementations',
        description: prependAlwaysReadReminder('Save local implementations (handler/saver classes) for a class. Workflow: Lock class → Save /includes/implementations → keep LOCKED. Used for RAP behavior handler classes (lhc_*, lsc_*). Use adt_activate to unlock and activate.'),
        inputSchema: {
          type: 'object',
          properties: {
            class_name: {
      ...
      {
        name: 'thin_v2_check_syntax',
        description: prependAlwaysReadReminder('Check syntax of a saved ABAP object. Returns errors, warnings, and info messages with line numbers.'),
        inputSchema: {
          type: 'object',
          properties: {
            object_name: {
      ...
      {
        name: 'thin_v2_check_syntax_unsaved',
        description: prependAlwaysReadReminder('Check syntax of unsaved ABAP source code WITHOUT saving to SAP. Validates AI-generated or modified code before committing. Source is sent as Base64-encoded content. Use this to validate before saving.'),
        inputSchema: {
          type: 'object',
          properties: {
            object_name: {
      ...
      {
        name: 'thin_v2_run_tests',
        description: prependAlwaysReadReminder('Run ABAP Unit tests for a class. Executes all test classes and returns detailed results including pass/fail status, execution times, and error messages.'),
        inputSchema: {
          type: 'object',
          properties: {
            object_name: {
            ...
            client: {
              type: 'string',
              description: 'Optional: SAP client to run tests in (e.g., "300"). If not provided, uses the default configured client. Useful for running tests with data from different clients.'
            }
          },
      ...
      {
        name: 'thin_v2_execute_class',
        description: prependAlwaysReadReminder('Execute runnable class (F9 functionality). Runs the if_oo_adt_classrun~main method and returns the console output. The class must implement the if_oo_adt_classrun interface and be activated.'),
        inputSchema: {
          type: 'object',
          properties: {
            class_name: {
            ...
            client: {
              type: 'string',
              description: 'Optional: SAP client to execute in (e.g., "300"). If not provided, uses the default configured client. Useful for running tests with data from different clients (e.g., test or production data).'
            }
          },
          required: ['class_name']
        }
      },
      {
        ...
        description: prependAlwaysReadReminder('📊 Execute SQL SELECT Query - Direct data preview (F8 functionality). Executes read-only SQL SELECT statements and returns structured results with column metadata. Perfect for data validation, testing, exploration, and cross-system data comparison. Supports WHERE clauses, JOINs, aggregations, and all standard SQL SELECT features. Returns up to specified number of rows with column types and descriptions.'),
        inputSchema: {
          type: 'object',
          properties: {
            sql_query: {
              type: 'string',
              description: 'SQL SELECT statement to execute. Must be a SELECT statement (read-only). Supports: WHERE clauses, JOINs, ORDER BY, GROUP BY, aggregations, subqueries, CDS views, database tables. Example: "SELECT matnr, maktx FROM mara WHERE mtart = \'FERT\' UP TO 10 ROWS"'
            },
            max_rows: {
              type: 'number',
              description: 'Maximum number of rows to return. Default: 100. Range: 1-10000. Use lower values for quick checks, higher values for data analysis.',
              default: 100
            },
            client: {
              type: 'string',
              description: 'Optional: SAP client to execute query in (e.g., "210", "300"). If not provided, uses the default configured client. Useful for querying data from different clients or environments.'
            }
          },
          required: ['sql_query']
        }
      ...
      {
        name: 'thin_v2_activate',
        description: prependAlwaysReadReminder('Unlock and activate ABAP objects. Workflow: Unlock → Activate. Supports BATCH activation - unlock and activate multiple objects in one operation. Use this after saving one or more objects with adt_save_source.'),
        inputSchema: {
          type: 'object',
          properties: {
            objects: {
      ...
      {
        name: 'thin_v2_update_and_activate',
        description: prependAlwaysReadReminder('Complete workflow for SINGLE object: Lock → Save → Unlock → Syntax Check → Activate. Use this for quick single-object updates. For batch operations, use adt_save_source (multiple times) then adt_activate.'),
        inputSchema: {
          type: 'object',
          properties: {
            object_name: {
      ...
      {
        name: 'thin_v2_where_used_list',
        description: prependAlwaysReadReminder('Find where an ABAP object is being used (where-used list). Returns all references to the specified object across the system. Useful for impact analysis before making changes.'),
        inputSchema: {
          type: 'object',
          properties: {
            object_name: {
      ...
      {
        name: 'thin_v2_list_package_objects',
        description: prependAlwaysReadReminder('List all objects in a package or filter by object type (DTEL, CLAS, DOMA, TABL, etc.). Returns object names, types, descriptions, and URIs.'),
        inputSchema: {
          type: 'object',
          properties: {
            package_name: {
              type: 'string',
              description: 'Package name (e.g., /COREVIST/BTP_HC, $TMP, ZPACKAGE)'
            },
            object_type: {
              type: 'string',
              description: 'Optional: Filter by object type. Leave empty to get all objects. Supported types: CLAS/CLASS, INTF/INTERFACE, DTEL/DATA_ELEMENT, DOMA/DOMAIN, TABL/TABLE, TTYP/TABLE_TYPE, DDLS/CDS, PROG/PROGRAM, FUGR/FUNCTION_GROUP, BDEF/BEHAVIOR_DEFINITION'
            }
          },
      ...
      {
        name: 'thin_v2_reassign_package',
        description: prependAlwaysReadReminder('Reassign an ABAP object to a new package. This is a refactoring operation that moves the object from its current package to a new package and updates the transport request.'),
        inputSchema: {
          type: 'object',
          properties: {
            object_name: {
      ...
      {
        name: 'thin_v2_unlock',
        description: prependAlwaysReadReminder('Unlock ABAP objects. Releases locks on objects so they can be modified by other users or in other sessions. Supports batch unlocking - unlock multiple objects in one operation. Use this when you want to unlock objects without activating them.'),
        inputSchema: {
          type: 'object',
          properties: {
            objects: {
      ...
      {
        name: 'thin_v2_remove_objects_from_transport',
        description: prependAlwaysReadReminder('Remove objects from a transport request or task. This allows you to remove objects that were added to a transport request/task.'),
        inputSchema: {
          type: 'object',
          properties: {
            transport_number: {
      ...
      {
        name: 'thin_v2_add_objects_to_transport',
        description: prependAlwaysReadReminder('Add ABAP objects to a transport request or task. Essential for BTP systems where objects are not automatically added during save operations. Use this after creating or modifying objects to add them to a transport.'),
        inputSchema: {
          type: 'object',
          properties: {
            transport_number: {
              type: 'string',
              description: 'Transport request or task number (e.g., H01K900138)'
            },
            objects: {
              type: 'array',
              description: 'List of objects to add to the transport',
              items: {
                type: 'object',
                properties: {
                  name: {
                    type: 'string',
                    description: 'Object name (e.g., /COREVIST/BTP_ACC_DATA_INV_SCM)'
                  },
                  type: {
                    type: 'string',
                    description: 'Object type (e.g., CLAS, TABL, DDLS, BDEF, etc.)'
                  },
                  pgmid: {
                    type: 'string',
                    description: 'Program ID (default: R3TR)',
      ...
      case 'thin_v2_switch_system': {
        ...
        if (!validSystems.includes(targetSystem)) {
          return {
            content: [{ 
              type: 'text', 
              text: `❌ **Invalid System**\n\nSystem "${args.system}" is not valid. Choose from: ${validSystems.join(', ')}`
            }]
          };
        }
        ...
        try {
          ...
          SERVER_ID = SAP_CONFIG.serverId;
          logFile = path.join(__dirname, `adt_debug_${SAP_CONFIG.systemKey.toLowerCase()}.log`);
          log = createLogger(SERVER_ID, logFile);
          
          // Update ADT service configuration
          adtService.config = SAP_CONFIG;
          adtService.log = log;
          adtService.isBtpMode = SAP_CONFIG.authMode === 'btp';
          ...
          // Reload BTP cookies if switching to BTP
          if (SAP_CONFIG.authMode === 'btp') {
            const btpCookies = loadBtpCookiesWrapper();
            if (btpCookies.length > 0) {
              btpCookies.forEach(cookieString => {
                ...
                adtService.cookies.set(cookieName, cookieValue);
              });
              log(`[SYSTEM SWITCH] Loaded ${btpCookies.length} BTP cookies`);
            } else {
              log('[SYSTEM SWITCH] No BTP cookies found');
            }
          }
          
          // Update axios client with new baseURL and auth
          adtService.client.defaults.baseURL = SAP_CONFIG.baseUrl;
          adtService.client.defaults.headers['sap-client'] = SAP_CONFIG.client;
          adtService.client.defaults.headers['sap-language'] = SAP_CONFIG.language;
          
          if (SAP_CONFIG.authMode === 'basic') {
            adtService.client.defaults.auth = {
              username: SAP_CONFIG.username,
              password: SAP_CONFIG.password
            };
            delete adtService.client.defaults.headers['Authorization'];
          } else if (SAP_CONFIG.oauthToken) {
            adtService.client.defaults.headers['Authorization'] = `Bearer ${SAP_CONFIG.oauthToken}`;
            delete adtService.client.defaults.auth;
          }
          
          log(`[SYSTEM SWITCH] ✅ Switched to ${targetSystem}`);
          
          return {
            content: [{ 
              type: 'text', 
              text: `✅ **System Switched Successfully!**

**Active System:** ${SAP_CONFIG.systemKey} (${SAP_CONFIG.serverId})
**Base URL:** ${SAP_CONFIG.baseUrl}
**Client:** ${SAP_CONFIG.client}
**Auth Mode:** ${SAP_CONFIG.authMode}

All subsequent tool calls will now use the **${targetSystem}** system.

**💡 Tip:** You can switch back anytime using:
- \`adt_switch_system("DEV")\`
- \`adt_switch_system("QA")\`
- \`adt_switch_system("BTP")\``
            }]
          };
        } catch (error) {
          return {
            content: [{ 
              type: 'text', 
              text: `❌ **Failed to Switch System**\n\n**Error:** ${error.message}`
            }]
          };
        }
      }

      case 'thin_v2_create_class': {
...
`;
      ...
      case 'thin_v2_create_data_element': {
        const options = {};
...
`;
      ...
      case 'thin_v2_save_domain': {
        const result = await adtService.saveDomain(
          args.domain_xml,
          args.transport_request
        );
        ...
        let responseText;
        if (result.success) {
          responseText = `✅ **Successfully Saved Domain ${result.domainName}**

**Workflow Completed:**
1. ✅ Domain locked
2. ✅ Domain saved with complete XML structure
3. ✅ Domain unlocked

**Domain updated with:**
- Type information (data type, length, decimals)
- Output information (formatting options)
- Value information (fixed values, value table)

**Next Steps:**
Activate the domain using \`adt_activate\`:
\`\`\`
adt_activate({ objects: [{ name: "${result.domainName}", type: "DOMA" }] })
\`\`\`

**Status:** 🎉 Domain definition saved and ready to activate!
`;
        } else {
          responseText = `❌ **Failed to Save Domain**

**Error:** ${result.error}
**HTTP Status:** ${result.httpStatus || 'Unknown'}

**Possible reasons:**
- Domain is locked by another user
- Invalid XML structure
- Transport request doesn't exist or is locked
- No authorization to change the domain

**Details:**
${result.details ? JSON.stringify(result.details, null, 2) : 'No additional details'}

**Hint:** ${result.hint || 'Check domain lock status and XML structure'}
`;
        }
      ...
      case 'thin_v2_save_data_element': {
        const result = await adtService.saveDataElement(
          args.data_element_xml,
          args.transport_request
        );

        let responseText;
        if (result.success) {
          responseText = `✅ **Successfully Saved Data Element ${result.dataElementName}**

**Workflow Completed:**
1. ✅ Data element locked
2. ✅ Data element saved with complete XML structure
3. ✅ Data element unlocked

**Data element updated with:**
- Domain reference (typeKind/typeName)
- Field labels (short, medium, long, heading)
- Data type information

**Next Steps:**
Activate the data element using \`adt_activate\`:
\`\`\`
adt_activate({ objects: [{ name: "${result.dataElementName}", type: "DTEL" }] })
\`\`\`

**Status:** 🎉 Data element definition saved and ready to activate!
`;
        } else {
          responseText = `❌ **Failed to Save Data Element**

**Error:** ${result.error}
**HTTP Status:** ${result.httpStatus || 'Unknown'}

**Possible reasons:**
- Data element is locked by another user
- Invalid XML structure
- Transport request doesn't exist or is locked
- No authorization to change the data element
- Domain reference doesn't exist

**Details:**
${result.details ? JSON.stringify(result.details, null, 2) : 'No additional details'}

**Hint:** ${result.hint || 'Check data element lock status and XML structure'}
`;
        }
      ...
      case 'thin_v2_save_table_type': {
        const result = await adtService.saveTableType(
          args.table_type_xml,
          args.transport_request
        );
        ...
        let responseText;
        if (result.success) {
          responseText = `✅ **Successfully Saved Table Type ${result.tableTypeName}**

**Workflow Completed:**
1. ✅ Table type locked
2. ✅ Table type saved with complete XML structure
3. ✅ Table type unlocked

**Table type updated with:**
- Row type definition (typeKind, typeName)
- Access type (standard/sorted/hashed)
- Primary key definition
- Secondary keys configuration

**Next Steps:**
Activate the table type using \`adt_activate\`:
\`\`\`
adt_activate({ objects: [{ name: "${result.tableTypeName}", type: "TTYP" }] })
\`\`\`

**Status:** 🎉 Table type definition saved and ready to activate!
`;
        } else {
          responseText = `❌ **Failed to Save Table Type**

**Error:** ${result.error}
**HTTP Status:** ${result.httpStatus || 'Unknown'}

**Possible reasons:**
- Table type is locked by another user
- Invalid XML structure
- Transport request doesn't exist or is locked
- No authorization to change the table type
- Referenced types (row type, range type) don't exist

**Details:**
${result.details ? JSON.stringify(result.details, null, 2) : 'No additional details'}

**Hint:** ${result.hint || 'Check table type lock status and XML structure'}
`;
        }
      ...
      case 'thin_v2_create_table_type': {
        const options = {};
...
`;
      ...
      case 'thin_v2_create_service_definition': {
        const options = {};
        if (args.ddl_source) options.ddlSource = args.ddl_source;
...
`;
      ...
      case 'thin_v2_create_service_binding': {
        const options = {};
...
`;
      ...
      case 'thin_v2_read_service_binding': {
...
`;
      ...
      case 'thin_v2_create_behavior_definition': {
...
`;
      ...
      case 'thin_v2_create_metadata_extension': {
...
`;
      ...
      case 'thin_v2_generate_rap_ui_service': {
        const options = {};
...
`;
      ...
      case 'thin_v2_generate_custom_query': {
        const options = {};
...
`;
      ...
      case 'thin_v2_read_source': {
        const result = await adtService.readSource(args.object_name, args.object_type);
        
        // DEBUG: Show which system was used
        const systemInfo = `\n\n---\n**🔍 DEBUG INFO:**\n- **🖥️ Server:** ${SAP_CONFIG.serverName} (${SAP_CONFIG.systemKey})\n- **System:** ${SAP_CONFIG.baseUrl}\n- **Auth Mode:** ${SAP_CONFIG.authMode}\n- **Client:** ${SAP_CONFIG.client}`;
        
        // Escape underscores in object names for Markdown rendering
        const escapedObjectName = (result.objectName || args.object_name).replace(/_/g, '\\_');
        
        let responseText;
        if (result.success) {
          const functionGroupInfo = result.functionGroup ? `\n**Function Group:** ${result.functionGroup}` : '';
          responseText = `✅ **Successfully Read ${result.objectType} ${escapedObjectName}**${functionGroupInfo}
...
`;
          ...
          const escapedObjectTypeArg = args.object_type.replace(/_/g, '\\_');
          const escapedObjectNameArg = args.object_name.replace(/_/g, '\\_');
          responseText = `❌ **Failed to Read ${escapedObjectTypeArg} ${escapedObjectNameArg}**
...
`;
      ...
      case 'thin_v2_save_testclass_source': {
...
`;
      ...
      case 'thin_v2_read_testclass_source': {
...
`;
      ...
      case 'thin_v2_read_local_implementations': {
...
`;
      ...
      case 'thin_v2_save_local_implementations': {
...
`;
      ...
      case 'thin_v2_check_syntax_unsaved': {
...
`;
      ...
      case 'thin_v2_execute_class': {
        const result = await adtService.executeClass(args.class_name, args.client);
...
`;
      ...
      case 'thin_v2_execute_sql_query': {
        const maxRows = args.max_rows || 100;
        const result = await adtService.executeSqlQuery(args.sql_query, maxRows, args.client);

        let responseText;
        if (result.success) {
          // Format the results in a nice table
          const columnHeaders = result.columns.map(c => c.name).join(' | ');
          const separator = result.columns.map(c => '---').join(' | ');
          
          // Build table rows (limit display to avoid overwhelming output)
          const displayRows = result.rows.slice(0, 20); // Show first 20 rows
          ...
          const metadataTable = result.columns.map(col => 
            `- **${col.name}** (${col.type})${col.description ? `: ${col.description}` : ''}`
          ).join('\n');

          responseText = `✅ **SQL Query Executed Successfully**

**Query Summary:**
- Total Rows Found: **${result.totalRows}**
...
${result.rows.length < result.totalRows ? `\n⚠️ **Note:** Showing ${result.rows.length} of ${result.totalRows} rows. Use \`max_rows\` parameter to get more rows (up to 10,000).\n` : ''}

**Executed Query:**
\`\`\`sql
${result.executedQuery || result.originalQuery}
\`\`\`

🎉 Query executed successfully!`;
        } else {
          responseText = `❌ **SQL Query Execution Failed**

**Error:** ${result.error}
**HTTP Status:** ${result.httpStatus || 'Unknown'}

**Hint:** ${result.hint || 'Check SQL syntax and ensure you have authorization to read the data'}

**Query:**
\`\`\`sql
${result.sqlQuery || args.sql_query}
\`\`\`
...
`;
      ...
      case 'thin_v2_update_and_activate': {
...
`;
      ...
      case 'thin_v2_list_package_objects': {
        const result = await adtService.listPackageObjects(
          args.package_name,
          args.object_type
        );

        let responseText = `📦 **Package Objects: ${args.package_name}**\n\n`;
        ...
        responseText += `**Filter:** ${result.objectType}\n`;
        responseText += `**Total Objects Found:** ${result.objectCount}\n\n`;

        if (result.objects && result.objects.length > 0) {
          responseText += `**Objects:**\n\n`;
          
          // Group by type if showing all objects
          if (!args.object_type) {
            ...
            result.objects.forEach(obj => {
              ...
              groupedObjects[type].push(obj);
            });
            
            Object.keys(groupedObjects).sort().forEach(type => {
              responseText += `### ${type} (${groupedObjects[type].length} objects)\n\n`;
              groupedObjects[type].forEach(obj => {
                responseText += `- **${obj.name}** - ${obj.description || 'No description'}\n`;
              });
              responseText += `\n`;
            });
          } else {
            // Just list objects if filtered by type
            result.objects.forEach((obj, index) => {
              responseText += `${index + 1}. **${obj.name}** - ${obj.description || 'No description'}\n`;
              if (obj.package) {
                responseText += `   - Package: ${obj.package}\n`;
              }
            });
          }
        } else {
          responseText += `✅ **No objects found** in package ${args.package_name}`;
          if (args.object_type) {
            responseText += ` of type ${args.object_type}`;
          }
          responseText += `.\n`;
        }
...
`;
      ...
      case 'thin_v2_remove_objects_from_transport': {
...
`;
      ...
      case 'thin_v2_add_objects_to_transport': {
        const result = await adtService.addObjectsToTransport(
          args.transport_number,
          args.objects
        );
        ...
        // Check if there are results (individual object processing)
        if (result.results && result.results.length > 0) {
          // Individual object results available
          const totalObjects = result.results.length;
          const successCount = result.results.filter(r => r.success).length;
          const errorCount = result.results.filter(r => !r.success).length;
          ...
          if (errorCount === 0) {
            // All successful
            responseText = `✅ **Successfully Added ${successCount} Object(s) to Transport ${args.transport_number}**

**Added Objects:**
${result.results.map(r => `- ✅ ${r.object} (Position: ${r.position || 'N/A'}, Lock Status: ${r.lockStatus || 'N/A'})`).join('\n')}

🎉 All objects have been added to the transport successfully!
`;
          } else {
            // Some errors occurred
            responseText = `⚠️ **Addition Completed with Issues**

**Transport:** ${args.transport_number}
**Results:**
- ✅ Successfully added: ${successCount} object(s)
- ❌ Errors: ${errorCount} object(s)

**Detailed Results:**
${result.results.map(r => {
  if (r.success) {
    return `✅ ${r.object} - Added successfully (Position: ${r.position || 'N/A'})`;
  } else {
    return `❌ ${r.object} - ${r.error || 'Failed to add'}`;
  }
}).join('\n')}
...
${result.results.filter(r => !r.success).map(r => {
  ...
  if (r.details) {
    // Extract package and message number if available
    const details = r.details;
    if (details.properties?.entry) {
      const entries = Array.isArray(details.properties.entry) ? details.properties.entry : [details.properties.entry];
      const msgIdEntry = entries.find(e => e['@_key'] === 'T100KEY-ID');
      const msgNoEntry = entries.find(e => e['@_key'] === 'T100KEY-NO');
      const msgId = msgIdEntry?.['#text'] || msgIdEntry;
      const msgNo = msgNoEntry?.['#text'] || msgNoEntry;
      if (msgId && msgNo && typeof msgId === 'string' && typeof msgNo !== 'object') {
        errorMsg += `\n**SAP Message:** ${msgId}/${msgNo}`;
      }
    }
  }
  return errorMsg;
...
`;
          ...
          // Operation failed entirely (no individual results)
          responseText = `❌ **Failed to Add Objects to Transport ${args.transport_number}**
...
`;
          ...
          // Success with no results (shouldn't happen, but handle it)
          return {
