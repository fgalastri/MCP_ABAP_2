...
│
├─ CHECK where object is used?
│  └─ Use: adt_where_used_list (impact analysis)
│
├─ CREATE new class?
...
│  └─ Use: adt_unlock (standalone unlock)
│
...
- `FUNC` or `FUNCTION_MODULE` - Function Module (with auto-discovery)
- `TABL` or `TABLE` - Database Table
- `STRUCT` or `STRUCTURE` - Structure
- `DTEL` or `DATA_ELEMENT` - Data Element (for where-used)
- `DOMA` or `DOMAIN` - Domain (for where-used)

**Example Prompts:**
- ✅ "Show me the code for class ZCL_TEST"
- ✅ "Read the source of program ZMCP_DEMO"
- ✅ "Read the include SAPWLSTAD_VB"
- ✅ "Read function module RFC_READ_TABLE"
- ✅ "Show me the BAPI_USER_GET_DETAIL function module"
- ✅ "What's in interface ZIF_MY_INTERFACE?"
- ✅ "Read structure ZS_ADDRESS"
- ✅ "Show me table ZTTMAT"

**Special Notes:**
- 🌟 **Function Modules:** Auto-discovery! No need to specify function group
- 📦 **Structures:** DDL format like tables, but no persistence
...
### 2️⃣ `adt_where_used_list` - Impact Analysis 🔍

**When to use:**
- Before modifying or deleting an object
- User asks "where is this used?" or "what depends on this?"
...
**Example Prompts:**
- ✅ "Where is class ZCL_TEST used?"
- ✅ "What references this CDS view?"
- ✅ "Check impact before deleting ZIF_OLD"
- ✅ "Find all dependencies of ZTT_MATERIAL"
- ✅ "What uses this table?"

**Agent Response Pattern:**
```
I'll check where ZCL_TEST is used in the system.

[Call adt_where_used_list]

🔍 **Where-Used List: CLAS ZCL_TEST**

**Total References Found:** 3

**Referenced Objects:**
1. **ZCE_PRODUCT** (DDLS/DF)
   - Package: ZMYPACK
   - Usage: gradeDirect,includeProductive

2. **ZCL_PROCESSOR** (CLAS/OC)
   - Package: ZMYPACK

⚠️ **Impact Warning:** This class is used by 3 objects.
Changes may affect these dependent objects.
```
...
### 3️⃣ `adt_save_source` - Save and Check (Keeps Locked)
...
### 4️⃣ `adt_check_syntax` - Check for Errors
...
### 5️⃣ `adt_unlock` - Standalone Unlock

**Workflow:** Unlock object(s) without activating

**When to use:**
- User wants to unlock objects without activating
- Release locks after saving with `adt_save_source`
- Unlock objects locked in other sessions
- Clean up locks after errors
- **Batch unlocking** - unlock multiple objects in one operation
...
**Optional Parameters:**
- `lock_handle` (string): Lock handle from previous lock operation (usually not needed)

**Returns:**
- Number of successfully unlocked objects
- Objects that were already unlocked (not an error)
- Error details if any unlock failed

**Example Prompts:**
- ✅ "Unlock class ZCL_TEST"
- ✅ "Release locks on these objects"
- ✅ "Unlock without activating"

**Agent Response Pattern:**
```
I'll unlock the objects for you.

[Call adt_unlock]

✅ **Successfully Processed 2 Object(s)**
...
🎉 All objects are now unlocked and ready for modification!
```

**Special Notes:**
- ✅ Gracefully handles already-unlocked objects (treated as success)
- ⚠️ Objects must be locked by current user (or provide lock_handle)
- 🌟 Supports batch unlocking - unlock multiple objects at once

---

### 6️⃣ `adt_activate` - Unlock and Activate
...
### 7️⃣ `adt_update_and_activate` - Complete Workflow ⭐
...
| "Where is this used?" | `adt_where_used_list` | "Where is ZCL_TEST used?" |
...
| "Unlock" | `adt_unlock` | "Unlock these objects" |
...
When modifying code:
- [ ] **Check where-used list** for impact analysis (especially for public APIs)
