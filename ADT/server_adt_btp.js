...
const logFile = path.join(__dirname, 'adt_debug_btp.log');
const SERVER_ID = '🌩️ BTP';
function log(message) {
  try {
    const timestamp = new Date().toISOString();
    const logMessage = `[${timestamp}] [${SERVER_ID}] ${message}\n`;
    fs.appendFileSync(logFile, logMessage);
    console.error(`[${SERVER_ID}] ${message}`);
  } catch (err) {
...
function parseAdtError(errorData) {
  ...
  if (typeof errorData === 'string') {
    ...
    if (errorData.trim().startsWith('<?xml') || errorData.trim().startsWith('<')) {
      try {
        const parsed = xmlParserBTP.parse(errorData);
...
// SAP ADT Connection Configuration - BTP VERSION
// This server is HARDCODED for BTP to avoid environment variable conflicts
const SAP_CONFIG_BTP = {
  baseUrl: process.env.SAP_BASE_URL || 'https://72bf6203-9328-4888-af10-ea65eeb72d78.abap-web.eu10.hana.ondemand.com',
  username: null, // Not used in BTP mode
  password: null, // Not used in BTP mode
  client: process.env.SAP_CLIENT || '100',
  // BTP Authentication - FORCED to 'btp' mode
  authMode: 'btp', // Always BTP for this server
  oauthToken: process.env.SAP_OAUTH_TOKEN || null, // Bearer token for BTP systems (optional)
  btpCookiesFile: process.env.SAP_BTP_COOKIES_FILE || path.join(__dirname, 'btp_cookies.json'), // Path to BTP cookies file
  language: process.env.SAP_LANGUAGE || 'EN'
};

// Log the configuration at startup
console.error('═══════════════════════════════════════════════════');
console.error('🚀 ABAP ADT MCP Server - BTP VERSION');
console.error('═══════════════════════════════════════════════════');
console.error(`📡 Base URL: ${SAP_CONFIG_BTP.baseUrl}`);
console.error(`🔐 Auth Mode: BTP (hardcoded)`);
console.error(`👤 Client: ${SAP_CONFIG_BTP.client}`);
console.error(`🍪 Cookies File: ${SAP_CONFIG_BTP.btpCookiesFile}`);
console.error('═══════════════════════════════════════════════════');

// Validate BTP configuration
if (!SAP_CONFIG_BTP.baseUrl) {
  console.error('❌ ERROR: SAP_BASE_URL environment variable is required!');
...
function loadBtpCookies() {
  try {
    if (SAP_CONFIG_BTP.authMode === 'btp' && fs.existsSync(SAP_CONFIG_BTP.btpCookiesFile)) {
      const cookiesData = JSON.parse(fs.readFileSync(SAP_CONFIG_BTP.btpCookiesFile, 'utf8'));
      ...
      if (cookies && cookies.length > 0) {
        log(`[CONFIG] Loaded ${cookies.length} cookies from ${SAP_CONFIG_BTP.btpCookiesFile}`);
        return cookies;
      }
    }
  } catch (error) {
    log(`[CONFIG] Warning: Failed to load BTP cookies: ${error.message}`);
  }
  return [];
}

// Initialize XML parser - BTP version
const xmlParserBTP = new XMLParser({
  ignoreAttributes: false,
  attributeNamePrefix: '@_',
  parseAttributeValue: true
});

const xmlBuilderBTP = new XMLBuilder({
  ignoreAttributes: false,
  attributeNamePrefix: '@_',
  format: true
});
...
class AdtService {
  constructor() {
    ...
    this.isBtpMode = SAP_CONFIG_BTP.authMode === 'btp';
    // Generate persistent connection ID (like Eclipse ADT does)
    // This allows SAP to track editing sessions and create proper local history entries
    this.connectionId = randomUUID().replace(/-/g, '');
    ...
    if (this.isBtpMode) {
      ...
      if (btpCookies.length > 0) {
        ...
        log(`[CONFIG] Pre-loaded ${btpCookies.length} BTP cookies`);
      } else {
        log('[CONFIG] No BTP cookies found - will attempt session creation');
      }
    }
    ...
    const axiosConfig = {
      baseURL: SAP_CONFIG_BTP.baseUrl,
      headers: {
        'sap-client': SAP_CONFIG_BTP.client,
        'sap-language': SAP_CONFIG_BTP.language,
    ...
    if (this.isBtpMode) {
      ...
      if (SAP_CONFIG_BTP.oauthToken) {
        axiosConfig.headers['Authorization'] = `Bearer ${SAP_CONFIG_BTP.oauthToken}`;
      }
      log('[CONFIG] Using BTP authentication mode');
    } else {
      // On-premise mode: Use basic auth
      axiosConfig.auth = {
        username: SAP_CONFIG_BTP.username,
        password: SAP_CONFIG_BTP.password
      };
      log('[CONFIG] Using basic authentication mode (on-premise)');
    }
    ...
    this.client.interceptors.request.use(request => {
      ...
      // Add ADT session headers (like Eclipse ADT does)
      // Connection ID: persistent across the session (allows SAP to track editing sessions and create local history)
      request.headers['sap-adt-connection-id'] = this.connectionId;
      // Request ID: unique per request (for request tracking)
      ...
      log(`Full URL: ${request.baseURL}${request.url}`);
      ...
      if (request.data && typeof request.data === 'string' && request.data.length < 500) {
        ...
        log(`Body length: ${request.data.length} bytes`);
      }
      log(`========== REQUEST END ==========\n`);
    ...
    this.client.interceptors.response.use(
      response => {
        ...
        if (response.data && typeof response.data === 'string' && response.data.length < 500) {
          ...
          log(`Response Body length: ${typeof response.data === 'string' ? response.data.length : JSON.stringify(response.data).length} bytes`);
        }
        log(`========== RESPONSE END ==========\n`);
      ...
      error => {
        ...
        log(`[${error.response?.status || 'NO STATUS'}] ${error.config?.method?.toUpperCase()} ${error.config?.url}`);
  ...
  async initialize() {
    ...
    try {
      if (this.isBtpMode) {
        ...
        log('[INIT] BTP Mode: Initializing...');
        
        // If we have pre-loaded cookies, skip session creation
        if (this.cookies.size > 0) {
          log('[INIT] Using pre-loaded cookies from btp_cookies.json');
          log('[INIT] Skipping session creation - reusing existing Eclipse session');
        } else {
          // No cookies - attempt to create session (requires browser auth)
          log('[INIT] No cookies available - attempting session creation...');
          log('[INIT] Warning: This may require browser authentication');
          ...
          try {
            ...
            log(`[INIT] BTP session created, status: ${sessionResponse.status}`);
            
            // Parse session XML to extract security session link
            const sessionXml = xmlParserBTP.parse(sessionResponse.data);
            ...
            if (sessionLink && sessionLink['@_href']) {
              ...
              log(`[INIT] Security session ID: ${this.securitySessionId}`);
            }
          } catch (error) {
            log('[INIT] Failed to create BTP session - likely needs browser authentication');
            log('[INIT] Please extract cookies from Eclipse ADT and save to btp_cookies.json');
        ...
        log('[INIT] BTP Mode: Getting CSRF token...');
        try {
          ...
          if (error.response && error.response.headers['x-csrf-token']) {
            log('[INIT] Got CSRF token from error response (expected behavior)');
        ...
        log('[INIT] On-Premise Mode: Getting reentrance ticket...');
        ...
        log(`[INIT] Reentrance ticket obtained: ${this.reentranceTicket.substring(0, 20)}...`);
        
        // Step 2: Get CSRF token from repository endpoint (may return 400 but still has token!)
        log('[INIT] On-Premise Mode: Getting CSRF token...');
        try {
          const csrfResponse = await this.client.get('/sap/bc/adt/repository/informationsystem/virtualfolders/contents', {
            headers: { 
              'X-CSRF-Token': 'Fetch'
      ...
      log(`[INIT] Initialization complete. CSRF token: ${this.csrfToken}`);
  ...
  async createTableType(tableTypeName, description, packageName, transportRequest, options = {}) {
    try {
      ...
      const {
        language = 'EN',
        masterLanguage = 'EN',
        responsible = SAP_CONFIG_BTP.username,
        lineType = null,
...
${fieldsDdl}
  ...
  async readServiceBinding(bindingName, retryCount = 0) {
    try {
      ...
      const metadata = xmlParserBTP.parse(response.data);
  ...
  <adtcore:packageRef adtcore:name="${packageName}"/>
      ...
      const lockData = xmlParserBTP.parse(lockResponse.data);
...
<dtel:wbobj xmlns:dtel="http://www.sap.com/wbobj/dictionary/dtel" xmlns:adtcore="http://www.sap.com/adt/core" adtcore:description="${description}" adtcore:language="${language}" adtcore:name="${dataElementName.toUpperCase()}" adtcore:type="DTEL/DE" adtcore:masterLanguage="${masterLanguage}" adtcore:masterSystem="S4H" adtcore:responsible="${responsible}">
  ...
  async readSource(objectName, objectType, functionGroup = null) {
    try {
      ...
      log(`[READ SOURCE] 🔐 Auth Mode: ${SAP_CONFIG_BTP.authMode}`);
      log(`[READ SOURCE] ═══════════════════════════════════════`);
...
<class:abapClass xmlns:class="http://www.sap.com/adt/oo/classes" xmlns:adtcore="http://www.sap.com/adt/core" adtcore:description="${description}" adtcore:language="${language}" adtcore:name="${className.toUpperCase()}" adtcore:type="CLAS/OC" adtcore:masterLanguage="${masterLanguage}" adtcore:masterSystem="S4H" adtcore:responsible="${responsible}" class:final="${final}" class:visibility="${visibility}">
  ...
  async createTable(tableName, description, packageName, transportRequest, options = {}) {
    try {
      ...
      const xmlRequest = `<?xml version="1.0" encoding="UTF-8"?><blue:blueSource xmlns:blue="http://www.sap.com/wbobj/blue" xmlns:adtcore="http://www.sap.com/adt/core" adtcore:description="${description}" adtcore:language="EN" adtcore:name="${tableName.toUpperCase()}" adtcore:type="TABL/DT" adtcore:masterLanguage="EN" adtcore:masterSystem="S4H" adtcore:responsible="${SAP_CONFIG_BTP.username}">
  <adtcore:packageRef adtcore:name="${packageName}"/>
</blue:blueSource>`;
  ...
  async createPackage(packageName, description, transportRequest, options = {}) {
    try {
      ...
      const {
        ...
        masterLanguage = 'EN',
        masterSystem = 'H01',
        responsible = 'CB9980000040', // Default BTP user ID - can be overridden
  ...
  <pak:applicationComponent/>`;
      ...
      // Add transport information - ALWAYS required by SAP systems
      xmlRequest += `
  <pak:transport>`;
  ...
  <pak:subPackages/>
      ...
      // Check if session expired (HTML response instead of XML)
      if (error.response?.data && typeof error.response.data === 'string' && error.response.data.includes('<html')) {
        return {
          success: false,
          error: 'BTP Session has expired. Please re-authenticate using: node ADT/authenticate_btp_simple.js',
          httpStatus: error.response?.status,
          rawError: 'HTML authentication redirect page received'
        };
      }
  ...
  async lockObject(objectName, objectType) {
    try {
      ...
      const lockData = xmlParserBTP.parse(response.data);
  ...
  <adtcore:objectSets xmlns:adtcore="http://www.sap.com/adt/core">
    <objectSet kind="inclusive">
      ...
      // Parse XML response
      const result = xmlParserBTP.parse(response.data);
  ...
  async executeClass(className) {
    try {
      ...
      log(`[EXECUTE CLASS] Full URL: ${this.client.defaults.baseURL}${executeUrl}`);
      log(`[EXECUTE CLASS] CSRF Token: ${this.csrfToken ? 'Present' : 'MISSING'}`);
      log(`[EXECUTE CLASS] Cookies: ${this.cookies.size} cookies set`);
      ...
      log(`[EXECUTE CLASS] SUCCESS: Response status ${response.status}`);
      log(`[EXECUTE CLASS] Response Headers: ${JSON.stringify(response.headers, null, 2)}`);
      log(`[EXECUTE CLASS] SAP Server: ${response.headers['sap-server']}`);
      log(`[EXECUTE CLASS] SAP Cache Control: ${response.headers['sap-cache-control']}`);
      log(`[EXECUTE CLASS] SAP Perf FESRec: ${response.headers['sap-perf-fesrec']}`);
      log(`[EXECUTE CLASS] Content-Length: ${response.headers['content-length']}`);
      log(`[EXECUTE CLASS] Response Body Length: ${response.data?.length || 0}`);
      log(`[EXECUTE CLASS] Response Body Preview (first 200 chars): ${response.data?.substring(0, 200) || 'empty'}`);
      ...
      return {
        ...
        debugInfo: {
          ...
          sapServer: response.headers['sap-server'],
          sapCacheControl: response.headers['sap-cache-control'],
          sapPerfFesrec: response.headers['sap-perf-fesrec']
        }
      };
    } catch (error) {
  ...
  /**
   * Remove objects from transport request or task
   * This allows removing objects that were added to a transport request/task
   * 
   * @param {string} transportNumber - Transport request/task number (e.g., H01K900028)
   * @param {Array} objects - Array of objects to remove, each with: { name, type, pgmid, obj_desc, position }
  ...
  async removeObjectsFromTransport(transportNumber, objects) {
    try {
      ...
      for (const obj of objects) {
        try {
          log(`[REMOVE FROM TRANSPORT] Removing object: ${obj.name} (${obj.type}/${obj.pgmid})`);
  ...
  async checkSyntaxUnsaved(objectName, objectType, sourceCode) {
    try {
      ...
      const xmlRequest = xmlBuilderBTP.build(checkRequest);
      ...
      const checkResult = xmlParserBTP.parse(response.data);
  ...
  async activateObjects(objects) {
    try {
      ...
      const xmlRequest = xmlBuilderBTP.build(activationRequest);
      ...
      // Parse XML response
      const activationResult = xmlParserBTP.parse(response.data);
  ...
  <usagereferences:affectedObjects/>
      ...
      // Parse XML response
      const parsedResponse = xmlParserBTP.parse(response.data);
  ...
  async reassignPackage(objectName, objectType, currentPackage, newPackage, transportRequest, description = '') {
    try {
      ...
      // Check if current package is local (starts with $)
      const isLocalPackage = currentPackage.startsWith('$');
      
      // For local packages, use empty transport. For transportable packages, use provided transport
      const transportValue = isLocalPackage ? '' : transportRequest;
...
<generic:genericRefactoring xmlns:adtcore="http://www.sap.com/adt/core" xmlns:generic="http://www.sap.com/adt/refactoring/genericrefactoring">
  ...
  <generic:transport>${transportValue}</generic:transport>
...
// Initialize ADT service - BTP version
const adtServiceBTP = new AdtService();

// Create MCP server - BTP version
const serverBTP = new Server(
  {
    name: 'abap-adt-service-btp',
    version: '1.0.0',
  },
  {
    capabilities: {
...
// Register tools
serverBTP.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      {
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            transport_request: {
              type: 'string',
              description: 'Transport request number (e.g., H01K900090). Leave empty for local packages.'
            },
            super_package: {
              type: 'string',
            ...
            responsible: {
              type: 'string',
              description: 'Optional: Person responsible (user ID). Defaults to CB9980000040 for BTP.'
            }
          },
...
serverBTP.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    const { name, arguments: args } = request.params;

    switch (name) {
      case 'adt_create_class': {
        const result = await adtServiceBTP.createClass(
          args.class_name,
          args.description,
          args.package_name,
...
`;
      ...
      case 'adt_create_table': {
        const result = await adtServiceBTP.createTable(
          args.table_name,
          args.description,
          args.package_name,
          args.transport_request,
...
`;
      ...
      case 'adt_create_package': {
        const result = await adtServiceBTP.createPackage(
          args.package_name,
          args.description,
          args.transport_request || '',
          {
...
${result.transportRequest ? `- Transport: ${result.transportRequest}` : '- Transport: (local package)'}
...
`;
          ...
          const parsedError = parseAdtError(result.details);
          const isSessionExpired = result.error && result.error.includes('BTP Session has expired');
...
${isSessionExpired ? '- BTP session has expired - please re-authenticate' : ''}

${isSessionExpired ? '💡 **Tip:** Run: node ADT/authenticate_btp_simple.js to refresh cookies' : ''}
`;
        }
      ...
      case 'adt_create_interface': {
        const result = await adtServiceBTP.createInterface(
          args.interface_name,
          args.description,
          args.package_name,
          args.transport_request
...
`;
      ...
      case 'adt_create_program': {
        const result = await adtServiceBTP.createProgram(
          args.program_name,
          args.description,
          args.package_name,
          args.transport_request,
...
`;
      ...
      case 'adt_create_cds_view': {
        ...
        const result = await adtServiceBTP.createCdsView(
          args.cds_name,
          args.description,
          args.package_name,
          args.transport_request,
...
`;
      ...
      case 'adt_create_data_element': {
        ...
        const result = await adtServiceBTP.createDataElement(
...
`;
      ...
      case 'adt_create_domain': {
        const result = await adtServiceBTP.createDomain(
          args.domain_name,
          args.description,
          args.package_name,
          args.transport_request,
...
`;
      ...
      case 'adt_create_table_type': {
        ...
        const result = await adtServiceBTP.createTableType(
...
`;
      ...
      case 'adt_create_structure': {
        ...
        const result = await adtServiceBTP.createStructure(
          args.structure_name,
          args.description,
          args.package_name,
          args.transport_request,
...
`;
      ...
      case 'adt_create_service_definition': {
        ...
        const result = await adtServiceBTP.createServiceDefinition(
          args.service_name,
          args.description,
          args.package_name,
          args.transport_request || '',
...
`;
      ...
      case 'adt_create_service_binding': {
        ...
        const result = await adtServiceBTP.createServiceBinding(
          args.binding_name,
          args.description,
...
`;
      ...
      case 'adt_read_service_binding': {
        const result = await adtServiceBTP.readServiceBinding(args.binding_name);
...
`;
      ...
      case 'adt_create_behavior_definition': {
        const result = await adtServiceBTP.createBehaviorDefinition(
          args.bdef_name,
          args.description,
          args.package_name,
          args.transport_request || '',
...
`;
      ...
      case 'adt_create_metadata_extension': {
        const result = await adtServiceBTP.createMetadataExtension(
...
`;
      ...
      case 'adt_generate_rap_ui_service': {
        ...
        const result = await adtServiceBTP.generateRapUiService(
          args.table_name,
          args.package_name,
          args.transport_request || '',
          options
...
### 5. **Test the Service**
   ...
   \`\`\`
   ${SAP_CONFIG_BTP.baseUrl}${result.generated.serviceBindingUri}
   \`\`\`
...
`;
      ...
      case 'adt_generate_custom_query': {
        ...
        const result = await adtServiceBTP.generateCustomQuery(
          args.entity_name,
...
`;
      ...
      case 'adt_read_source': {
        const result = await adtServiceBTP.readSource(args.object_name, args.object_type);
        
        // DEBUG: Show which system was used
        const systemInfo = `\n\n---\n**🔍 DEBUG INFO:**\n- **System:** ${SAP_CONFIG_BTP.baseUrl}\n- **Auth Mode:** ${SAP_CONFIG_BTP.authMode}\n- **Client:** ${SAP_CONFIG_BTP.client}`;
...
`;
      ...
      case 'adt_save_source': {
        ...
        try {
          // Step 1: Lock
          const lockResult = await adtServiceBTP.lockObject(args.object_name, args.object_type);
          ...
          // Step 2: Save
          const saveResult = await adtServiceBTP.saveSource(
            args.object_name,
            args.object_type,
            args.source_code,
          ...
          const syntaxResult = await adtServiceBTP.checkSyntax(args.object_name, args.object_type, 'inactive');
...
`;
          ...
          if (lockHandle) {
            try {
              await adtServiceBTP.unlockObject(args.object_name, args.object_type, lockHandle);
...
`;
      ...
      case 'adt_save_testclass_source': {
        ...
        try {
          ...
          const lockResult = await adtServiceBTP.lockObject(args.class_name, 'CLAS');
          steps.push({ step: 'lock', result: lockResult });
          ...
          const saveResult = await adtServiceBTP.saveTestClassInclude(
            args.class_name,
            args.source_code,
            lockHandle,
            transportRequest
          ...
          const syntaxResult = await adtServiceBTP.checkSyntax(args.class_name, 'CLAS', 'inactive');
          steps.push({ step: 'syntax_check', result: syntaxResult });

          // Step 4: Try to read back what was saved (for verification)
          const readBackResult = await adtServiceBTP.readTestClassInclude(args.class_name);
...
`;
          ...
          if (lockHandle) {
            try {
              await adtServiceBTP.unlockObject(args.class_name, 'CLAS', lockHandle);
...
`;
      ...
      case 'adt_read_testclass_source': {
        const result = await adtServiceBTP.readTestClassInclude(args.class_name);
...
`;
      ...
      case 'adt_read_local_implementations': {
        const result = await adtServiceBTP.readLocalImplementations(args.class_name);
...
`;
      ...
      case 'adt_save_local_implementations': {
        ...
        try {
          ...
          const saveResult = await adtServiceBTP.saveLocalImplementations(
            args.class_name,
            args.source_code,
            lockHandle,
            transportRequest
...
`;
      ...
      case 'adt_check_syntax': {
        const result = await adtServiceBTP.checkSyntax(
          args.object_name,
          args.object_type,
...
`;
      ...
      case 'adt_check_syntax_unsaved': {
        const result = await adtServiceBTP.checkSyntaxUnsaved(
          args.object_name,
          args.object_type,
          args.source_code
        );
...
`;
      ...
      case 'adt_run_tests': {
        const result = await adtServiceBTP.runUnitTests(
          args.object_name,
...
`;
      ...
      case 'adt_execute_class': {
        const result = await adtServiceBTP.executeClass(args.class_name);
...
`;
      ...
      case 'adt_activate': {
        ...
        for (const obj of args.objects) {
          try {
            const unlockResult = await adtServiceBTP.unlockObject(obj.name, obj.type, null);
        ...
        const result = await adtServiceBTP.activateObjects(args.objects);
...
`;
      ...
      case 'adt_update_and_activate': {
        const result = await adtServiceBTP.updateAndActivate(
          args.object_name,
          args.object_type,
          args.source_code
        );
...
`;
      ...
      case 'adt_where_used_list': {
        const result = await adtServiceBTP.whereUsedList(
          args.object_name,
          args.object_type
        );
      ...
      case 'adt_reassign_package': {
        const result = await adtServiceBTP.reassignPackage(
          args.object_name,
          args.object_type,
      ...
      case 'adt_unlock': {
        ...
        for (const obj of args.objects) {
          try {
            ...
            const unlockResult = await adtServiceBTP.unlockObject(obj.name, obj.type, lockHandle);
...
`;
      ...
      case 'adt_remove_objects_from_transport': {
        const result = await adtServiceBTP.removeObjectsFromTransport(
...
async function main() {
  console.error('═══════════════════════════════════════════════════');
  console.error('🚀 ABAP ADT MCP Server - 🌩️ BTP VERSION 🌩️');
  console.error('═══════════════════════════════════════════════════');
  console.error(`📡 Base URL: ${SAP_CONFIG_BTP.baseUrl}`);
  console.error(`🔐 Auth Mode: ${SAP_CONFIG_BTP.authMode.toUpperCase()} (hardcoded)`);
  console.error(`👤 Client: ${SAP_CONFIG_BTP.client}`);
  console.error(`🍪 Cookies File: ${SAP_CONFIG_BTP.btpCookiesFile}`);
  console.error(`🌐 Language: ${SAP_CONFIG_BTP.language}`);
  console.error('═══════════════════════════════════════════════════');
  
  const transport = new StdioServerTransport();
  await serverBTP.connect(transport);
  
  console.error('✅ BTP ADT MCP Server ready for connections');
}
